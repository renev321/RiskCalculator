import math
    "based on a fixed percentage risk per trade."
)
...
with st.sidebar:
    ...
    blowup_threshold = st.slider(
        ...
        value=10,
        step=1,
        help="Example: 10% means we count the account as basically blown once only 10% of the original balance remains.",
    )
...
if risk_decimal >= 1:
    st.error("Risk per trade cannot be 100% or more if you want a meaningful progressive-loss calculation.")
else:
    if risk_decimal <= 0:
        st.warning("Please use a risk greater than 0%.")
    else:
        # Solve: initial * (1-risk)^n <= threshold
        # n >= log(threshold/initial) / log(1-risk)
        ...
        st.info(
            ...
            f"it would take about **{losses_to_threshold} consecutive losses** to fall to **{blowup_threshold}%** "
            f"of the original balance."
        )
        ...
        if losses_to_threshold <= max_losses_to_show:
            threshold_row = next((row for row in balances if row["Loss Number"] == losses_to_threshold), None)
            if threshold_row:
                st.success(
                    f"At loss #{losses_to_threshold}, the balance would be about "
                    f"${threshold_row['Remaining Balance ($)']:,.2f} ({threshold_row['Remaining (%)']}% remaining)."
                )
        else:
            st.warning(
                "The threshold was not reached within the current simulation range. Increase 'Max losses to simulate' to see it in the table."
            )
